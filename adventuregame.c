    ...
    typedef struct {
        char name[50];
        int attack_power; // For weapons
        int heal_amount;  // For healing items
    } Item;
   ...
   void save_game(Player *player, Room *rooms, int current_room_index) {
    ...
    if (file) {
        ...
        for (int i = 0; i < 5; i++) {
            fwrite(&rooms[i], sizeof(Room) - sizeof(Monster *), 1, file); // Pointer hariç her şeyi yaz
            if (rooms[i].monster != NULL) {
                int monster_index = rooms[i].monster - monsters; // Monster pointer'ı yerine indeks kaydet
...
void load_game(Player *player, Room *rooms, int *current_room_index) {
    ...
    if (file) {
        ...
        // 5 odanın her birinin verilerini yükle
        for (int i = 0; i < 5; i++) {
            fread(&rooms[i], sizeof(Room) - sizeof(Monster *), 1, file);

            // Canavar indeksini oku ve canavarı odada ayarla
            int monster_index;
        ...
        // Oyuncunun bulunduğu odanın indeksini oku
        ...
        // Eğer dosya açılamazsa, hata mesajı göster
    ...
    // Eğer odada bir bulmaca varsa ve çözülmemişse ipucunu göster
    ...
    // Eğer odada bir canavar varsa bilgilerini göster
    ...
    // Oyuncuya mevcut komutları hatırlat
