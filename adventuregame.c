    ...
    int main() {
       ...
       Room rooms[5] = {
        ...
        {"Bandage", "Knife", "Puzzle Paper"}, 3, &monsters[0], 1, "5 * Y = 15. What is Y?", 0, 1},
        {"You are in Room 2.",
        {"Axe", "Painkiller"}, 2, &monsters[1], 0, "", 0, 0},
        {"You are in Room 3. There are potions here. There is also a recipe for potion mixing: 'Red + Blue = Purple'.",
        {"Red Potion", "Blue Potion", "Yellow Potion", "Potion Recipe"}, 4, NULL, 0, "", 0, 0},
        {"You are in Room 4.",
        {"Katana"}, 1, &monsters[2], 0, "", 0, 0},
...
void load_game(Player *player, Room *rooms, int *current_room_index) {
    ...
    // Odaya ilk kez geçmek için sırayla keşfetme kontrolü
    if (target_room > *current_room_index && !rooms[target_room - 1].discovered) {
        printf("You cannot move to Room %d yet. Discover Room %d first.\n", target_room + 1, target_room);
        return;
    }
    ...
    *current_room_index = target_room;
    rooms[*current_room_index].discovered = 1; // Oda keşfedildi
